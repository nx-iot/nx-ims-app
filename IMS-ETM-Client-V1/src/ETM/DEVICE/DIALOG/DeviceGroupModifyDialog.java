...
public class DeviceGroupModifyDialog extends javax.swing.JDialog implements ActionListener, MouseListener
{
    ...
    public DeviceGroupModifyDialog(DeviceGroupPanel DGP)
    {
        this.DGP = DGP;
        initComponents();
        extendComponents();
    ...
    public DeviceGroupModifyDialog(DeviceGroupPanel DGP, int i)
    {
        this.DGP = DGP;
        initComponents();
        extendComponents();
    ...
    private void initComponents() {
        ...
        undoButton.setToolTipText("Return");
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        .addContainerGap()
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addComponent(ungroupPanel, javax.swing.GroupLayout.DEFAULT_SIZE, 238, Short.MAX_VALUE)
                            .addComponent(groupPanel, javax.swing.GroupLayout.DEFAULT_SIZE, 238, Short.MAX_VALUE)))
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        .addGap(52, 52, 52)
    ...
    {
        Vector v = DGP.DIF.MF.HDC.queryColumn("SELECT CONCAT('[B] ', device_name, ' (',device_id+0, ')') FROM device WHERE equipment_id = 1 AND device_group_id = '"+groupIDField.getText()+"';");
        Vector v2 = DGP.DIF.MF.HDC.queryColumn("SELECT CONCAT('[T] ', device_name, ' (',device_id+0, ')') FROM device WHERE equipment_id = 3 AND device_group_id = '"+groupIDField.getText()+"';");
        if (v2 != null)
        {
    ...
    {
        Vector v = DGP.DIF.MF.HDC.queryColumn("SELECT CONCAT('[B] ', device_name, ' (',device_id+0, ')') FROM device WHERE equipment_id = 1 AND (device_group_id IS NULL OR device_group_id = '');");
        Vector v2 = DGP.DIF.MF.HDC.queryColumn("SELECT CONCAT('[T] ', device_name, ' (',device_id+0, ')') FROM device WHERE equipment_id = 3 AND (device_group_id IS NULL OR device_group_id = '');");
        if (v2 != null)
        {
            for (int i=0;i<v2.size();i++)
            {
