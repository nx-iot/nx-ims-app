...
public class SMSLogPanel extends javax.swing.JPanel implements ActionListener
{
    ...
    public SMSLogPanel(LogManagementIF LMIF)
    {
        this.LMIF = LMIF;
    ...
    private void setAccessTable()
    {
        ...
        String opt = "(sms_log.success = 2 ";
        if (successCheckBox.isSelected())
        {
            opt += "OR sms_log.success = 1 ";
        }
        if (failCheckBox.isSelected())
        {
            opt += "OR sms_log.success = 0 ";
        }
        opt += ") AND ";
        ...
        String stmt = "SELECT "
                + "sms_log.user_username AS 'User', destination AS 'Address', provider_name AS 'Provider', message1 AS 'Text-1', message2 AS 'Text-2', "
                + "timestamp AS 'Send-Time', success AS 'Success' "
                + "FROM "+LMIF.MF.HDC.database+"_log.sms_log NATURAL JOIN user "
                + "WHERE "+groupFilter+opt+searchOption+between+" true ORDER BY sms_log.timestamp DESC;";
